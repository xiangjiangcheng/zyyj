		...
		<div style="margin:5px 5px">
			...
			<span class="sitem">
				...
				<input id="es_program" name="es_program" style="width:100px" class="easyui-combobox"
				 url="http://localhost/zyyj/index.php/Home/Tao/Tao_GetProgram"
				 valueField="program_id" textField="name" >
				</input>  
			</span>
			<span class="sitem">
				<label>等级</label>
				<input id="es_level" name='es_level' class='easyui-combobox' url="http://localhost/zyyj/index.php/Home/Tao/Tao_GetLevel" style="width:100px" valueField="level_id" textField="name"> 
			</span>
...
<script type="text/javascript">
	...
	$(function(){
		$('#es_program').combobox({
			onSelect:function(record){
				$('#es_level').combobox({
					url:'http://localhost/zyyj/index.php/Home/Tao/Tao_GetLevel?programid='+record.program_id,
					valueField:'level_id',
					textField:'name',
				});
			}
	...
	/*function loadbox(){
		/*$('#getcollege').combobox({
		    url:'<?php echo U('Tao/Tao_GetCollege');?>',
		    valueField:'college_id',
		    textField:'name',
		});  
		$('#getmajor').combobox({
		    url:'<?php echo U('Tao/Tao_GetMajor');?>',
		    valueField:'major_id',
		    textField:'name',
		}); 
		$('#getgrade').combobox({
		    url:'<?php echo U('Tao/Tao_GetGrade');?>',
		    valueField:'grade_id',
		    textField:'name',
		});
		$('#getclass').combobox({
		    url:'<?php echo U('Tao/Tao_GetClass');?>',
		    valueField:'class_id',
		    textField:'name',
		});
		$('#getcourse').combobox({
		    url:'<?php echo U('Tao/Tao_GetCourse');?>',
		    valueField:'course_id',
		    textField:'name',
		});
		$('#getprogram').combobox({
		    url:'<?php echo U('Tao/Tao_GetProgram');?>',
	...
	function Es_getexcel(){
		...
		window.open('http://localhost/zyyj/index.php/Home/ExamScore/ExamScore_Getexcel?grade_id='+gradeid+'&class_id='+classid+'&major_id='+majorid+'&course_id='+courseid+'&program_id='+programid+'&level_id'+levelid+'&stuname='+stuname+'&account='+account+'&status='+status);
	}
	/*$(function(){
		$('#getexcel').dialog({
			buttons:[{
				...
				handler:function(){
					if($('#socrelist').form('validate')){
						...
						window.open('http://localhost/zyyj/index.php/Home/ExamScore/ExamScore_Getexcel?grade_id='+gradeid+'&class_id='+classid+'&major_id='+majorid+'&course_id='+courseid+'&program_id='+programid+'&level_id'+levelid);
					}
				}
			},{
				text:'取消',
	...
	$(function () {
		$('#Es_table').datagrid({
			...
			title:'考试成绩记录表',
			iconCls:'icon-search',
			url:'<?php echo U('ExamScore/ExamScore_GetScores');?>',
			method: 'POST',
			singleSelect:true,
			rownumbers:true,
			fit:true,
