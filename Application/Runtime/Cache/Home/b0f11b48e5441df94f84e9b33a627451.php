...
<script type="text/javascript">
	...
	Obj = {
		...
		loadbox2:function(){
			...
			$('#Dcollege').combobox({
			    ...
			    onSelect:function(record){
			    	$('#Pdpt').combobox({
			    		url:'<?php echo U('SysUserManage_Dpt_GetDlist');?>'+'?college_id='+$('#Dcollege').combobox('getValue'),
		...
		insert2:function(){
			if($('#NewDpt').form("validate")){
				$.ajax({
					url:'<?php echo U('SysUserManage_Dpt_Insert');?>',
					type:'POST',
					data:{
						Dtype:$('#Dtype').val(),
				...
				$.messager.confirm('确定操作','删除之后将不可恢复，是否确认？',
					function(qr){
						if(qr){
							...
							$.ajax({
								url:'<?php echo U('SysUserManage_Dpt_delete');?>',
								type:'POST',
								data:{
									department_id:Did,
								},
	...
	$(function(){
		$('#Dtable').datagrid({
			...
			title:'部门信息表',
			iconCls:'icon-search',
			url:'<?php echo U('SysUserManage_Dpt_GetInfo');?>',
			method: 'POST',
			singleSelect:true,
			rownumbers:true,
			fit:true,
