...
<script type="text/javascript">
	...
	$('#s_college').combobox({
		...
		onSelect:function(rec){
			$('#Udept1').combobox({
				url:'{:U('SysUserManage_Dpt_GetDlist')}'+'?college_id='+$('#s_college').combobox('getValue'),
	...
	Mth1 ={
		loadbox:function(){
			$('#Upost').combobox({
			    url:'{:U('SysUserManage_GetPostInfo')}',
			    valueField:'post_id',
			    textField:'name',
			});
			$('#Udept').combobox({
				url:'__SITE__index.php/Home/SysUserManage/SysUserManage_GetbDlist',
				valueField:'department_id',
				textField:'name',
			...
			$('#Ucollege').combobox({
				...
				onSelect:function(record){
					$('#Udept').combobox({
						url:'__SITE__index.php/Home/SysUserManage/SysUserManage_GetbDlist?cid='+record.college_id,
						valueField:'department_id',
						textField:'name',
					});
		...
		insert1:function(){
			if ($('#NewUser').form('validate')){ 
				if($('#Udept').combobox('getValue')==-1){
					...
					$.ajax({
						url:'{:U('SysUserManage_User_Insert')}',
						type:'POST',
						data:{
		...
		xg1:function(){
			...
			if(rows.length>0){
				...
				$.messager.confirm('确定操作',msg,function(flag){
					if(flag){
						$.post('{:U('SysUserManage_SetStatus')}',{user_id:rows[0].user_id,status:rows[0].status},function(data){
			...
			if(rows.length>0){
				...
				function(flag){
					if(flag){
						...
						$.ajax({
							url:'{:U('SysUserManage_User_delete')}',
							type:'POST',
							data:{
								user_id:id,
							},
				...
				$.messager.confirm('确定操作','是否将该用户的密码重置为:123456？',
					function(flag){
						if(flag){
							var id=rows[0].user_id;
							$.ajax({
							url:'{:U('SysUserManage_User_Resetpwd')}',
							type:'POST',
							data:{
								user_id:id,
							},
