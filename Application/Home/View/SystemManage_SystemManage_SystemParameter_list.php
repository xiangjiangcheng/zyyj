...
<script type="text/javascript">
		...
		xg:function(){
			...
			if(rows.length==1){
				if (this.editRow!=index){
		    		$('#rollback,#save').show();
		    		$('#xg').hide();
					$('#Gtable').datagrid('endEdit', this.editRow); //结束对一行进行编辑。
				}
				if(this.editRow==undefined){
					$('#rollback,#save').show();
					$('#xg').hide();
					$('#Gtable').datagrid('beginEdit', index); //开始
					this.editRow = index; 
				}
			}else{
	...
	$(function(){
		...
		$('#Gtable').datagrid({
	    height: '100%',
	    url:'{:U('SystemManage_SystemParameter_list_show')}',
	    method: 'GET',
	    striped: true,
	    fitColumns: true,
	    singleSelect: true,
		...
		onAfterEdit:function(rowIndex,rowData,changes){
		    ...
		    if(updated.length>0){
			    $.ajax({
			    	url:'{:U('SystemManage_SystemParameter_edit')}',
					type:'POST',
					data:{
